    ...
    <div>
        ...
        </div>
        {
            state.get().items.filter(t => state.get().showComplete || !t.isDone).map((t, i) => <div>
                <label><input type="checkbox" checked={state.bind(s => s.items[i].isDone)} /> {t.task}</label>
            </div>)
        }
        {state.get().items.length == 0 ? <div>No Tasks</div> : ''}
        <div>
            <input value={state.bind(s => s.newTask)} placeholder="New Item" />
            <button onclick={e => state.mutate(s => s.items.push({task: s.newTask, isDone: false}))}>Add New</button>
        </div>
    </div>
)